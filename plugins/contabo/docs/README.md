...
- **Region codes used by `cntb`.** When creating instances, regions are specified as coarse slugs: `EU`, `US-central`, `US-east`, `US-west`, `SIN` (Singapore), `UK` (United Kingdom — Portsmouth), `JPN` (Japan), `AUS` (Australia), `IND` (India). Use `cntb get datacenters` to list available options.
